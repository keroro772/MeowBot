...
def db_setup(func):  # easy wrapper for setring up databases
    def inner(login_name, password, host_name, database):
        con = pymysql.connect(login_name, password, host_name, database)
        c = con.cursor()
        func(c)
        con.commit()
        con.close()
...
class Bot:
    ...
    def _get_stats(self):
        ...
        #     self.loop,
        #     "https://twitchemotes.com/api_cache/v3/global.json"))['emotes']
    ...
    def start(self, tasked=False):
        ...
        if tasked:
            self.loop.create_task(self.checkifrunning())
